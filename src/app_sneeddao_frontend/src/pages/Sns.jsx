...
function Sns() {
    ...
    // Current SNS banner state
    const [currentSnsInfo, setCurrentSnsInfo] = useState(null);
    const [currentSnsLogo, setCurrentSnsLogo] = useState(null);
    ...
    // Load current SNS info for banner
    useEffect(() => {
        if (!selectedSnsRoot || snsList.length === 0) {
            setCurrentSnsInfo(null);
            setCurrentSnsLogo(null);
            return;
        }

        const currentSns = snsList.find(sns => sns.rootCanisterId === selectedSnsRoot);
        if (currentSns) {
            setCurrentSnsInfo(currentSns);
            // Get logo from the logos map
            const logo = snsLogos.get(currentSns.canisters?.governance);
            setCurrentSnsLogo(logo || null);
        }
    }, [selectedSnsRoot, snsList, snsLogos]);
                ...
                <div style={{ maxWidth: '1400px', margin: '0 auto', position: 'relative', zIndex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem', marginBottom: '0.5rem' }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                            <h1 style={{ 
                                ...
                                {currentSnsInfo ? `${currentSnsInfo.name}` : 'SNS Directory'}
                                {currentSnsInfo && (
                                    <span style={{
                                        fontSize: '0.75rem',
                                        fontWeight: '500',
                            ...
                            <p style={{ 
                                ...
                                {currentSnsInfo 
                                    ? 'Browse all SNS DAOs or explore the selected project' 
                                    : 'Browse the SNS ecosystem, pick a DAO, and jump straight into proposals, neurons, and the forum'
                                }
                            </p>
                        </div>

                        {/* Quick Stats - inline on desktop */}
                        {!isMobile && (
                            <div style={{ display: 'flex', gap: '1.25rem', flexWrap: 'wrap', alignItems: 'center' }}>
                                ...
                                {currentSnsInfo && selectedSnsDetails && (
                                    <>
                    ...
                    {isMobile && (
                        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', marginTop: '0.5rem' }}>
                            ...
                            {currentSnsInfo && selectedSnsDetails && selectedSnsDetails.tokenSymbol && (
                                <div style={{ 
                                    display: 'flex', 
                                    alignItems: 'center', 
                                    gap: '6px',
            ...
            <main style={{
                ...
                overflow: isMobile ? 'auto' : 'hidden'
                    ...
                    <div style={{
                        ...
                        {/* Details Content - Scrollable on desktop */}
                        {!isDetailsCollapsed && (
                            <div
                                            ...
                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                                                ...
                                                {selectedSnsDetails.nervousSystemParameters && (
                                                    <div>
                                                        ...
                                                        {/* Neuron Permissions - Expandable - Always show when nervousSystemParameters exists */}
                                                        {(() => {
                                                            // Extract permissions using the same pattern as SnsJailbreak (which works correctly)
                                                            const nsParams = selectedSnsDetails.nervousSystemParameters;
                                                            ...
                                                            const claimerPerms = rawClaimer?.[0]?.permissions || rawClaimer?.permissions || [];
                                                            
                                                            // Convert any permission values (could be BigInt, regular number, or array-like)
                                                            // Don't use Array.isArray since Candid might return typed arrays or array-like objects
                                                            const normalizePerms = (perms) => {
                                                                if (!perms) return [];
                                                                // Convert to regular array (handles TypedArrays, array-like objects, etc.)
                                                                const arr = Array.from(perms);
                                                                return arr.map(p => {
                                                                    if (typeof p === 'bigint') return Number(p);
                                                                    if (typeof p === 'number') return p;
                                                                    if (typeof p === 'object' && p !== null) {
                                                                        // Handle possible enum variant objects like { Vote: null }
                                                                        const keys = Object.keys(p);
                                                                        if (keys.length === 1) {
                                                                            const permName = keys[0];
                                                                            const permMap = {
                                                                                'Unspecified': 0, 'ConfigureDissolveState': 1, 'ManagePrincipals': 2,
                                                                                'SubmitProposal': 3, 'Vote': 4, 'Disburse': 5, 'Split': 6,
                                                                                'MergeMaturity': 7, 'DisburseMaturity': 8, 'StakeMaturity': 9,
                                                                                'ManageVotingPermission': 10
                                                                            };
                                                                            return permMap[permName] ?? -1;
                                                                        }
                                                                    }
                                                                    return parseInt(p, 10);
                                                                }).filter(p => !isNaN(p) && p > 0); // Filter out NaN and 0 (Unspecified)
                                                            };
                                                                        ...
                                                                        <div style={{
                                                                            ...
                                                                            {!hasAnyPermissions ? (
                                                                                <div style={{ color: theme.colors.mutedText, fontStyle: 'italic' }}>
                                                                                    This SNS has not configured any grantable hotkey permissions.
                                                                                </div>
                                                                            ) : (
                                                                                <>
                                                                                    {normalizedGrantable.length > 0 && (
                                                                                        <div style={{ marginBottom: '1rem' }}>
                                                                                            ...
                                                                                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
                                                                                                {normalizedGrantable.map((permNum, idx) => {
                                                                                    ...
                                                                                    {normalizedClaimer.length > 0 && (
                                                                                        <div>
                                                                                            ...
                                                                                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
                                                                                                {normalizedClaimer.map((permNum, idx) => {
                                                                                                    const permInfo = PERMISSION_INFO[permNum];
                                                                                                    return (
                                                                                                        <span key={idx} style={{
                                                                                                            padding: '0.35rem 0.7rem',
                    ...
                    {/* SNS List - Shows second on mobile, scrolls independently on desktop */}
                    <div style={{
                        ...
                        {/* List Content - Scrollable on desktop */}
                        {!isListCollapsed && (
                            <div style={{
                                background: theme.colors.secondaryBg,
