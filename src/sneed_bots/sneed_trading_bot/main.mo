...
shared (deployer) persistent actor class TradingBotCanister() = this {
    ...
    /// Parse a balance key back into (token, optional subaccount number).
    /// Keys are always generated by balanceKey(), so the principal part is always valid.
    func parseBalanceKey(key: Text): ?(Principal, ?Nat) {
        let iter = Text.split(key, #char ':');
        let tokenPart = switch (iter.next()) { case (?t) t; case null { return null } };
        let subPart = switch (iter.next()) { case (?s) s; case null { return null } };
        let token = Principal.fromText(tokenPart);
        if (subPart == "main") { return ?(token, null) };
        var num: Nat = 0;
        for (c in subPart.chars()) {
    ...
    func getChorePurseBalance(instanceId: Text, token: Principal, subaccount: ?Blob): Nat {
        let key = balanceKey(token, subaccount);
    ...
    func setChorePurseBalance(instanceId: Text, token: Principal, subaccount: ?Blob, amount: Nat) {
        let key = balanceKey(token, subaccount);
    ...
    func adjustChorePurseBalance(instanceId: Text, token: Principal, subaccount: ?Blob, delta: Nat, subtract: Bool) {
        if (delta == 0) return;
        let current = getChorePurseBalance(instanceId, token, subaccount);
        ...
        setChorePurseBalance(instanceId, token, subaccount, newVal);
    };

    /// Sum all chore purse balances across every chore for a specific (token, subaccount).
    func sumAllChorePurseBalances(token: Principal, subaccount: ?Blob): Nat {
        let key = balanceKey(token, subaccount);
        var total: Nat = 0;
        for ((cid, entries) in chorePurseBalances.vals()) {
            for ((k, v) in entries.vals()) {
    ...
    func computeMainPurseBalance(token: Principal, subaccount: ?Blob, onChainBalance: Nat): { balance: Nat; overcommitted: Bool } {
        let allocated = sumAllChorePurseBalances(token, subaccount);
    ...
    func getEffectiveBalance(instanceId: Text, token: Principal, subaccount: ?Blob, onChainBalance: Nat): Nat {
        if (isPurseEnabledForChore(instanceId)) {
            let purse = getChorePurseBalance(instanceId, token, subaccount);
            Nat.min(purse, onChainBalance)
        } else {
            let main = computeMainPurseBalance(token, subaccount, onChainBalance);
    ...
    /// Read a balance that is purse-aware for a given (token, subaccount).
    /// If choreInstanceId is provided and that chore has a purse, read from the purse;
        ...
        switch (result) {
            case (#Ok(r)) {
                ...
                if (isPurseEnabledForChore(instanceId)) {
                    ...
                    adjustChorePurseBalance(instanceId, outputToken, null, netAmountOut, false);
                };
                true
            };
            case (#Err(e)) {
    ...
    func executeDeposit(action: T.ActionConfig, instanceId: Text): async* Bool {
        ...
        switch (result) {
            case (#Ok(blockIdx)) {
                ...
                if (isPurseEnabledForChore(instanceId)) {
                    adjustChorePurseBalance(instanceId, action.inputToken, null, amount + fee, true);
                    adjustChorePurseBalance(instanceId, action.inputToken, ?targetBlob, amount, false);
                };
                true
            };
            case (#Err(e)) {
    ...
    func executeWithdraw(action: T.ActionConfig, instanceId: Text): async* Bool {
        ...
        switch (result) {
            case (#Ok(blockIdx)) {
                ...
                if (isPurseEnabledForChore(instanceId)) {
                    adjustChorePurseBalance(instanceId, action.inputToken, ?sourceBlob, amount + fee, true);
                    adjustChorePurseBalance(instanceId, action.inputToken, null, amount, false);
                };
                true
            };
            case (#Err(e)) {
        ...
        switch (result) {
            case (#Ok(r)) {
                ...
                if (isPurseEnabledForChore(instanceId)) {
                    ...
                    adjustChorePurseBalance(instanceId, buyToken.token, null, netAmountOut, false);
                };
                true
            };
            case (#Err(e)) {
    ...
    func executeRebalance(instanceId: Text): async* Bool {
        ...
        for (target in targets.vals()) {
            ...
            reconcileBalance(target.token, null, onChainBal, src);
            let balance = getEffectiveBalance(instanceId, target.token, null, onChainBal);
        ...
        if (isPurseEnabledForChore(instanceId)) {
            adjustChorePurseBalance(instanceId, sellToken.token, null, tradeSize, true);
            adjustChorePurseBalance(instanceId, intermediary, null, intermediaryReceived, false);
        };
        ...
        switch (leg2Result) {
            case (#Ok(r)) {
                ...
                if (isPurseEnabledForChore(instanceId)) {
                    ...
                    adjustChorePurseBalance(instanceId, buyToken.token, null, netAmountOut2, false);
                };
                true
            };
            case (#Err(e)) {
                ...
                for ((key, bal) in entries.vals()) {
                    if (bal > 0) {
                        switch (parseBalanceKey(key)) {
                            case (?(tok, sub)) { buf.add({ token = tok; subaccountNumber = sub; balance = bal }) };
                            case null {};
                        };
                    };
                };
    ...
    public shared query (msg) func getPurseBalance(instanceId: Text, token: Principal, subaccountNumber: ?Nat): async Nat {
        assertPermission(msg.caller, T.TradingPermission.ViewPortfolio);
        let sub = switch (subaccountNumber) {
            case (?n) ?subaccountNumberToBlob(n);
            case null null;
        };
    ...
    public shared (msg) func getMainPurseBalance(token: Principal, subaccountNumber: ?Nat): async T.MainPurseBalance {
        assertPermission(msg.caller, T.TradingPermission.ViewPortfolio);
        let sub = switch (subaccountNumber) {
            case (?n) ?subaccountNumberToBlob(n);
            case null null;
    ...
    public shared (msg) func fundPurse(instanceId: Text, token: Principal, amount: Nat): async { #Ok; #Err: Text } {
        ...
        adjustChorePurseBalance(instanceId, token, null, amount, false);
    ...
    public shared (msg) func reclaimFromPurse(instanceId: Text, token: Principal, amount: Nat): async { #Ok; #Err: Text } {
        ...
        if (amount == 0) { return #Err("Amount must be > 0.") };
        let current = getChorePurseBalance(instanceId, token, null);
        if (amount > current) {
            return #Err("Insufficient purse balance. Available: " # Nat.toText(current) # ", requested: " # Nat.toText(amount));
        };
        adjustChorePurseBalance(instanceId, token, null, amount, true);
